...
public extension UIImage {
    public func toMTLTexture() -> MTLTexture? {
        if let imageRef = self.cgImage {
            ...
            let bitmapContext = CGContext.init(data: rawData, width: width, height: height, bitsPerComponent: bitsPerComponent, bytesPerRow: bytesPerRow, space: colorSpace, bitmapInfo: (CGBitmapInfo.byteOrder32Big.rawValue | CGImageAlphaInfo.premultipliedLast.rawValue))
            ...
            if let defaultDevice = MTLCreateSystemDefaultDevice(), let texture: MTLTexture = defaultDevice.makeTexture(descriptor: textureDescriptor), let data = rawData {
